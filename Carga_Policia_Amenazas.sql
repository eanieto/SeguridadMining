...
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') or "ARMAS MEDIOS" is null THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.amenazas_2017_0

UNION
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') or "ARMAS MEDIOS" is null THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.amenazas_2018_1
UNION
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') or "ARMAS MEDIOS" is null THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.amenazas_2019_1
UNION
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') or "ARMAS MEDIOS" is null THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.amenazas_2020_0
          ) AS e
...
SELECT
    DISTINCT
    CASE WHEN "AGRUPA EDAD PERSONA" IN('-','NO REPORTADO','NO REGISTRA') or "AGRUPA EDAD PERSONA" is null THEN 'Indeterminado'
        WHEN "AGRUPA EDAD PERSONA" = 'ADOLECENTES' THEN 'ADOLESCENTES'
        ELSE "AGRUPA EDAD PERSONA" end var_nombre_detalle
FROM sc_fuentes_data.amenazas_2017_0
UNION
SELECT
    DISTINCT
...
WITH hurtos as (
    SELECT "DEPARTAMENTO",
           ...
           "FECHA HECHO",
           CASE WHEN "GENERO" IN('-','NO REPORTADO','NO REGISTRA') or "GENERO" is null THEN 'Indeterminado' ELSE "GENERO" end"GENERO",
           CASE WHEN "AGRUPA EDAD PERSONA" IN('-','NO REPORTADO','NO REGISTRA',' ') or "AGRUPA EDAD PERSONA" is null THEN 'Indeterminado'
                WHEN "AGRUPA EDAD PERSONA" = 'ADOLECENTES' THEN 'ADOLESCENTES'
                ELSE "AGRUPA EDAD PERSONA" end  "AGRUPA EDAD PERSONA",
           "CANTIDAD"
    FROM sc_fuentes_data.amenazas_2017_0
    UNION ALL
    SELECT "DEPARTAMENTO",
...
left join metricas
    on metricas.var_nombre_metrica = 'Conteo de Amenazas'
           and (h."ARMAS MEDIOS" = metricas.var_nombre_detalle or h."GENERO" = metricas.var_nombre_detalle or h."AGRUPA EDAD PERSONA" = metricas.var_nombre_detalle)
           and metricas.var_nombre_etiqueta in ('Armas y medios','Grupo edad','Genero')
...
-- truncate table bodega.fact_analisis_metricas;
select * from bodega.fact_analisis_metricas
where sk_metrica in(2,3,4,7,14,17)
