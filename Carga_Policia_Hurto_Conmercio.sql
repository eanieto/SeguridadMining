...
/*
Carga de la dimension de Etiquetas-Hurto a comercio
*/
/*Armas y medios*/
...
FROM sc_fuentes_data.hurto_a_comercio_2017_0

UNION
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.hurto_a_comercio_2018_1
UNION
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.hurto_a_comercio_2018_2
UNION
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.hurto_a_comercio_2019_0
UNION
SELECT
    DISTINCT
    CASE WHEN "ARMAS MEDIOS" IN('-','NO REPORTADO') THEN 'Indeterminado' ELSE "ARMAS MEDIOS" end var_nombre_detalle
FROM sc_fuentes_data.hurto_a_comercio_2020_0
          ) AS e
...
where
var_nombre_etiqueta in('Armas y medios');
...
WITH divipola as (
    ...
    FROM sc_fuentes_data.hurto_a_comercio_2017_0 LETICIA (CT)
...
/*
Carga de hechos para los hurtos a comercios
  */
...
left join metricas
    on metricas.var_nombre_metrica = 'Conteo de Hurtos a Comercios'
           and h."ARMAS MEDIOS" = metricas.var_nombre_detalle
           and metricas.var_nombre_etiqueta = 'Armas y medios'
